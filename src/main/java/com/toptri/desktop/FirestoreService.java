...
public class FirestoreService {
    ...
    public boolean hasRecentSellerMessage(String requestId, long withinMs) throws ExecutionException, InterruptedException {
        Timestamp cutoff = Timestamp.ofTimeSecondsAndNanos(
            (System.currentTimeMillis() - withinMs) / 1000, 0
        );

        QuerySnapshot snap = db.collection("requests")
                .document(requestId)
                .collection("messages")
                .whereEqualTo("senderType", "SELLER")
                .whereGreaterThan("createdAt", cutoff)
                .get().get();

        // Pastikan bukan dari AI_BOT sendiri
        for (QueryDocumentSnapshot doc : snap.getDocuments()) {
            String senderId = safe(doc.getString("senderId"));
            if (!senderId.equals("AI_BOT")) return true;
        }
        return false;
    }

    public boolean hasMessages(String requestId) throws Exception {
